...
if __name__ == "__main__":
    ...
    mat_c = numpy.zeros((1, 1))

    mat_a[0][0] = (1.0/j_wheel)*((k/r)*(-k) -mu)
    
    mat_b[0][0] = (1.0/j_wheel)*(k/r)
    ...
    y_result[:, 0]*= 60.0/(2.0*numpy.pi)
    LibsControl.plot_open_loop_response(t_result, y_result, "results/open_loop_response", labels=["rpm"])
    

    
    lqg     = LibsControl.LQGISolver(ds.mat_a, ds.mat_b, ds.mat_c, q, r, w, dt)
    ...
    #required state
    xr = numpy.array([[w_max]]).T

    #step response
    u_result, x_result, x_hat_result, y_result  = lqg.closed_loop_response(xr, steps, disturbance=True)
    x_hat_result[:, 0]*= 60.0/(2.0*numpy.pi)
    y_result[:, 0]*= 60.0/(2.0*numpy.pi)
    LibsControl.plot_closed_loop_response(t_result, u_result, y_result, x_hat_result, file_name = "results/closed_loop_response.png", u_labels=["voltage [v]"], x_labels=["rpm"] )
