...
class TrackingKalmanFilter:
    ...
    def step(self, x_obs):
        ...
        self._adaptive_q(error, alpha = 0.02, update_k = (self.steps%32) == 0)
    ...
    def _adaptive_q(self, error, alpha = 0.1, update_k = False):
        # adaptive Q estimation
        alpha = 0.1 
        Q_emp = (error.T @ error) / self.n_objects
        ...
        '''
        w, V = numpy.linalg.eigh(Q_emp)
        w_clipped = numpy.clip(w, 0.0, None)
        Q_emp = (V * w_clipped) @ V.T
        '''
        
        self.Q_est = (1.0 - alpha)*self.Q_est + alpha*Q_emp
        
        if update_k:
            self.K = self._solve_kalman_gain(self.A, self.H, self.Q_est, self.R)
